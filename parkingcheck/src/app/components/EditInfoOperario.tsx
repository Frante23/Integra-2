...
const EditInfo = () => {
    ...
    const handleChangeName = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const OperatorPass = event.currentTarget.OperatorPass.value;

        try {
            const response = await fetch('/api/auth/changeNameOp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ NewName: newName, OperatorPass: OperatorPass }),
            });
    ...
    const handleChangeEmail = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch('/api/auth/changeEmailOp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ NewEmail: newEmail, OperatorPass: OperatorPass }),
            });
    ...
    const handleChangeProfilePicture = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch('/api/UploadOp', {
                method: 'POST',
                body: formData,
            });
    ...
    const handleChangePassword = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch('/api/auth/changeMypassOp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ NewPass: newPass, OperatorPass: OperatorPass }),
            });
    ...
    const handleDeleteAccount = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch('/api/auth/deleteaccountOp', { 
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ OperatorPass: OperatorPass }), 
            });
            ...
            {userInfo ? (
                <>
                    ...
                    <div className="Changer-div rounded-xl">
                        ...
                        <div className="p-3 text-lg">{userInfo.OperatorName}</div>
                        <button onClick={() => openPopup(
                            'Cambiar nombre',
                             <div>
                                <form onSubmit={handleChangeName}>
                                    <input type="text" name="NewName" id="NewName" placeholder="Por favor ingrese el nuevo nombre" required/>
                                    <input type="password" name="OperatorPass" id="OperatorPass" placeholder="Por favor ingrese la contraseña actual" required/>
                    ...
                    <div className="Changer-div rounded-xl">
                        ...
                        <div className="p-3 text-lg">{userInfo.OperatorEmail}</div>
                        <button onClick={() => openPopup(
                            'Cambiar email', 
                            <div>
                        ...
                        <button onClick={() => openPopup(
                            ...
                            <div>
                                <form onSubmit={handleChangePassword}>
                                    ...
                                    <input type="password" name="OperatorPass" id="OperatorPass" placeholder="Por favor ingrese la antigua contraseña" required/>
                                    <button type='submit'>Cambiar</button>
                                </form>
                            </div>
